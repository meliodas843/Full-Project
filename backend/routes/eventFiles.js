// routes/eventFiles.js ✅ FULL FIXED
// ✅ MULTI FILE upload
// ✅ Only AFTER event finished
// ✅ Creator OR Booked user can upload
// ✅ Better datetime parsing (MySQL DATETIME safe)
// ✅ Better error handling (multer + mysql)
...
/* =========================
   Date helpers (FIX MySQL DATETIME)
========================= */
function normalizeDateTime(dt) {
  ...
  // JS Date() prefers ISO: "YYYY-MM-DDTHH:MM:SS"
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (!isAllowedMime(file.mimetype)) return cb(new Error("File type not allowed"));
    cb(null, true);
  },
});
...
function makePublicUrl(req, storedName) {
  // if behind proxy, optionally set: app.set("trust proxy", 1)
...
router.get("/:id/files", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("GET /api/events/:id/files error:", err); // ✅ full error
...
/* =========================
   ...
   Upload MULTIPLE files ONLY if event is finished
========================= */
  ...
  upload.array("files", 10)(req, res, async (multerErr) => {
    ...
    if (multerErr) {
      ...
      if (multerErr.code === "LIMIT_FILE_SIZE") {
        return res.status(400).json({ message: "File too large (max 20MB each)" });
      }
      if (multerErr.code === "LIMIT_UNEXPECTED_FILE") {
        return res.status(400).json({ message: "Unexpected field. Use form-data key: files" });
      }
    ...
    try {
      ...
      if (!ev.end_time) {
        return res.status(400).json({ message: "Event has no end_time, cannot upload as 'finished'." });
      }
      if (!isFinished(ev.end_time)) {
        return res.status(400).json({ message: "You can upload files only after the event is finished." });
      }

      // Permission: creator OR booked user_id
      const isCreator =
        String(ev.created_by_email || "").toLowerCase() === String(userEmail).toLowerCase();
      ...
      if (!isCreator && !isBooked) {
        return res.status(403).json({ message: "Only event creator or booked users can upload files." });
      }
      ...
      return res.status(201).json({ message: "Files uploaded ✅", files: inserted });
    } catch (err) {
      console.error("POST /api/events/:id/files error:", err); // ✅ full error
...
router.delete("/:eventId/files/:fileId", authMiddleware, async (req, res) => {
  try {
    ...
    const [evRows] = await pool.query(`SELECT id, created_by_email FROM events WHERE id=? LIMIT 1`, [eventId]);
    if (!evRows.length) return res.status(404).json({ message: "Event not found" });
    ...
    const isCreator = String(ev.created_by_email || "").toLowerCase() === String(userEmail).toLowerCase();
    const isUploader = String(file.uploaded_by_email || "").toLowerCase() === String(userEmail).toLowerCase();

    if (!isCreator && !isUploader) {
      return res.status(403).json({ message: "Not allowed to delete this file" });
    }

    await pool.query(`DELETE FROM event_files WHERE id=? AND event_id=?`, [fileId, eventId]);
